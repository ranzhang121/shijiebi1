...
请针对给出的世界杯队伍，结合其近期战绩、伤病名单、即时赔率、裁判掏牌风格、天气状况等特征，生成一份今日比赛预测。
你必须将战术分析与玄学原理（卦象、五行相生相克、比赛时辰）有机结合。例如：
- 如果赔率异常或被低估，分析是否存在“机构诱盘”。
...
必须严格以 JSON 格式返回以下字段，不要包含任何 markdown 格式标记 (如 \`\`\`json)，也不要包含任何额外的换行或解释：
{
  ...
  "metaphysicalAnalysis": "结合赔率大盘诱盘风险、比赛时辰、五行卦象的玄学星盘点评（不超过80字）"
}`;
...
【即时赔率大盘 (${team.odds.bookmaker})】：
- 主胜: ${team.odds.homeWin}
- 平局: ${team.odds.draw}
- 客胜: ${team.odds.awayWin}
